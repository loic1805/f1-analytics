...
st.markdown(
    "**Multi-Driver Telemetry Comparison.** *Select up to 5 drivers to analyze speed and gaps.*"
)
...
with st.sidebar:
    ...
    if session_light:
        ...
        try:
            driver_options = sorted(
                session_light.results["Abbreviation"].dropna().unique().tolist()
            )
        except:
    ...
    if selected_drivers:
        ref_driver = st.selectbox(
            "Reference Driver (Baseline)", options=selected_drivers, index=0
        )
    else:
        ref_driver = None
    run_btn = st.button(
        "Analyze Telemetry", type="primary", disabled=not selected_drivers
    )
    ...
    for i, driver in enumerate(selected_drivers):
        if driver in drivers_data:
            ...
            if driver == ref_driver:
                cols[i].metric(
                    label=driver, value=lap_time, delta="Ref", delta_color="off"
                )
            else:
                # now let's compute the gap in final lap time
                gap = (
                    drivers_data[driver]["lapTime"]
                    - drivers_data[ref_driver]["lapTime"]
                ).total_seconds()
                cols[i].metric(
                    label=driver,
                    value=lap_time,
